...
export default function CabinScreen({
  ...
  return(
    ...
    >
      ...
      {/* 3. NOTEBOOK ON DESK — TEMP: World Map hidden — re-add later
      ...
      */}
      ...
      {bookChooser&&<div style={{position:"fixed",inset:0,zIndex:120}}>
        ...
        <div style={{position:"absolute",inset:0,zIndex:2,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:"clamp(18px,5vh,38px)",padding:"9% 14px 8%",overflowY:"auto"}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",alignItems:"flex-start",justifyContent:"center",gap:"clamp(12px,3.5vw,30px)",width:"100%",maxWidth:560}}>
            ...
            {/* ── Book 6: COLORING ── */}
            <div style={{width:"clamp(96px,27vw,172px)",display:"flex",flexDirection:"column",alignItems:"center",gap:12}}>
              <button onClick={()=>{setBookChooser(false);setPrevScreen&&setPrevScreen("cabin");setScreen("coloring");}} style={{width:"100%",aspectRatio:"2 / 3",borderRadius:"4px 11px 11px 4px",border:"none",padding:0,cursor:"pointer",position:"relative",overflow:"hidden",background:"linear-gradient(160deg,#2E2418 0%,#241B11 48%,#181109 100%)",boxShadow:"0 16px 44px rgba(0,0,0,0.6), 0 0 0 1px rgba(201,169,110,0.22)",animation:"fadeUp .5s ease both"}}>
                <img src="/coloringbook.png" alt="Coloring Book" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover"}} draggable={false}/>
                <div style={{position:"absolute",left:0,top:0,bottom:0,width:14,background:"linear-gradient(90deg,rgba(20,12,4,0.55),rgba(20,12,4,0.12),transparent)",pointerEvents:"none",zIndex:2}}/>
                <div style={{position:"absolute",inset:0,boxShadow:"inset 0 0 50px rgba(0,0,0,0.35)",borderRadius:"inherit",pointerEvents:"none"}}/>
              </button>
              <div style={{textAlign:"center",animation:"fadeUp .6s .1s ease both"}}>
      ...
      {medOpen&&<div style={{position:"fixed",inset:0,zIndex:125}}>
        ...
        <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:"min(88vw,420px)",height:"min(78vh,640px)",animation:"bookOpenAnim .5s cubic-bezier(.22,1,.36,1) both",display:"flex",flexDirection:"column"}}>
          ...
          {/* Cream page */}
          <div style={{flex:1,background:"linear-gradient(155deg,rgba(30,23,16,0.58) 0%,rgba(22,16,11,0.66) 50%,rgba(16,11,8,0.72) 100%)",backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)",border:"1px solid rgba(201,169,110,0.22)",borderRadius:"3px 10px 10px 3px",position:"relative",overflow:"hidden",boxShadow:"0 4px 30px rgba(0,0,0,0.4), inset 0 0 80px rgba(201,169,110,0.06)"}}>
            <div style={{position:"relative",zIndex:2,height:"100%",display:"flex",flexDirection:"column",alignItems:"center",padding:"30px 24px 24px",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
              ...
              <h2 style={{fontFamily:DISPLAY,fontSize:"clamp(1.4rem,5.5vw,1.75rem)",fontWeight:700,color:"#F0E0C0",margin:"0 0 6px",letterSpacing:"0.04em",textAlign:"center"}}>Meditations</h2>
              <p style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"0.82rem",color:"#E8DCC8",lineHeight:1.6,maxWidth:280,margin:"0 0 2px",textAlign:"center"}}>&ldquo;…in his law doth he meditate day and night.&rdquo;</p>
              <div style={{fontFamily:SANS,fontSize:"0.62rem",letterSpacing:"0.1em",color:"rgba(230,210,165,0.6)"}}>PSALM 1:2 · KJV</div>
              <div style={{width:54,height:1,background:"linear-gradient(90deg,transparent,#C9A96E,transparent)",margin:"16px 0 18px"}}/>
              <div style={{width:"100%",maxWidth:340,display:"flex",flexDirection:"column",gap:11}}>
                {[
                  ...
                  <button key={m.id} onClick={()=>{setMedOpen(false);setBookChooser(false);setScreen(m.id);}} style={{background:"rgba(201,169,110,0.06)",border:"1px solid rgba(201,169,110,0.18)",borderRadius:10,padding:"13px 16px",cursor:"pointer",textAlign:"left",transition:"all .2s"}} onMouseEnter={e=>e.currentTarget.style.background="rgba(201,169,110,0.12)"} onMouseLeave={e=>e.currentTarget.style.background="rgba(201,169,110,0.06)"}>
                    <div style={{fontFamily:DISPLAY,fontSize:"0.95rem",fontWeight:700,color:"#F0E0C0",marginBottom:2}}>{m.label}</div>
                    <div style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"0.76rem",color:"rgba(230,210,165,0.55)"}}>{m.desc}</div>
                  </button>
                ))}
              </div>
              <button onClick={()=>{setMedOpen(false);setChooserIdx(1);setBookChooser(true);}} style={{marginTop:20,background:"linear-gradient(135deg,rgba(93,74,46,0.1),rgba(93,74,46,0.04))",border:"1px solid rgba(93,74,46,0.22)",color:"#E8DCC8",padding:"10px 26px",borderRadius:8,fontFamily:SERIF,fontStyle:"italic",fontSize:"0.82rem",cursor:"pointer"}}>&#8249; Back to your books</button>
            </div>
          </div>
        </div>
      </div>}
      ...
      {bookOpen&&<div style={{position:"fixed",inset:0,zIndex:100}}>
        ...
        <div onTouchStart={bookTouchStart} onTouchEnd={bookTouchEnd} style={{position:"absolute",top:"50%",left:"50%",width:"min(88vw,420px)",height:"min(78vh,640px)",animation:"bookOpenAnim .5s cubic-bezier(.22,1,.36,1) both",display:"flex",flexDirection:"column"}}>
          ...
          {/* Cream page */}
          <div key={`p-${bookPage}-${deskBook}`} style={{flex:1,background:"linear-gradient(155deg,rgba(30,23,16,0.58) 0%,rgba(22,16,11,0.66) 50%,rgba(16,11,8,0.72) 100%)",backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)",border:"1px solid rgba(201,169,110,0.22)",borderRadius:"3px 10px 10px 3px",position:"relative",overflow:"hidden",animation:`${flipDir==="bwd"?"pageRevealBwd":flipDir==="fwd"?"pageRevealFwd":"pageInitial"} .45s ease-out both`,boxShadow:"0 4px 30px rgba(0,0,0,0.4), 0 0 60px rgba(0,0,0,0.15), inset -2px 0 6px rgba(201,169,110,0.06)"}}>
            {/* Paper texture */}
            <div style={{position:"absolute",inset:0,background:"repeating-linear-gradient(0deg,transparent,transparent 28px,rgba(201,169,110,0.04) 28px,rgba(201,169,110,0.04) 29px)",pointerEvents:"none",borderRadius:"inherit"}}/>
            <div style={{position:"absolute",inset:0,boxShadow:"inset 0 0 80px rgba(201,169,110,0.12), inset 0 0 30px rgba(201,169,110,0.06)",pointerEvents:"none",borderRadius:"inherit"}}/>
              ...
              {deskBook==="journal"&&<>
                ...
                {bookPage===1&&!journalSection&&<>
                  <div style={{flex:1,display:"flex",flexDirection:"column",animation:"pageContentReveal .5s .1s ease both"}}>
                    <div style={{textAlign:"center",marginBottom:18}}>
                      <h2 style={{fontFamily:DISPLAY,fontSize:"clamp(1.15rem,4.5vw,1.35rem)",fontWeight:700,color:"#F0E0C0",margin:"0 0 4px"}}>Choose Your Path</h2>
                      <div style={{width:50,height:1,background:"linear-gradient(90deg,transparent,#C9A96E,transparent)",margin:"8px auto 0"}}/>
                    </div>
                ...
                {journalSection==="blank"&&bookPage===2&&<>
                  <div style={{flex:1,display:"flex",flexDirection:"column",animation:"pageContentReveal .5s .1s ease both"}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                      <button onClick={()=>{setJournalSection(null);setBookPage(1);setFlipDir("bwd");}} style={{background:"transparent",border:"none",cursor:"pointer",fontFamily:SERIF,fontSize:"0.74rem",color:"rgba(230,210,165,0.5)",padding:0}}>&#8249; Contents</button>
                    </div>
                    <h2 style={{fontFamily:DISPLAY,fontSize:"clamp(1.05rem,4vw,1.2rem)",fontWeight:700,color:"#F0E0C0",margin:"0 0 10px",textAlign:"center"}}>Past Entries</h2>
                    <div style={{width:40,height:1,background:"linear-gradient(90deg,transparent,rgba(201,169,110,0.3),transparent)",margin:"0 auto 12px"}}/>
                ...
                {journalSection==="blank"&&bookPage===3&&<>
                  <div style={{flex:1,display:"flex",flexDirection:"column",animation:"pageContentReveal .5s .1s ease both"}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
                      <button onClick={()=>{setJournalSection(null);setBookPage(1);setFlipDir("bwd");}} style={{background:"transparent",border:"none",cursor:"pointer",fontFamily:SERIF,fontSize:"0.74rem",color:"rgba(230,210,165,0.5)",padding:0}}>&#8249; Contents</button>
                      <div style={{flex:1}}/>
                      <span style={{fontFamily:SANS,fontSize:"0.58rem",color:"rgba(230,210,165,0.35)",letterSpacing:"0.1em",textTransform:"uppercase"}}>{todayStr()}</span>
                    </div>
                    <h2 style={{fontFamily:DISPLAY,fontSize:"clamp(1.1rem,4vw,1.3rem)",fontWeight:700,color:"#F0E0C0",margin:"0 0 12px",textAlign:"center"}}>Free Write</h2>
                    <div style={{width:40,height:1,background:"linear-gradient(90deg,transparent,rgba(201,169,110,0.3),transparent)",margin:"0 auto 14px"}}/>
                    <div style={{background:"rgba(201,169,110,0.04)",border:"1px solid rgba(201,169,110,0.1)",borderRadius:10,overflow:"hidden",flex:1,minHeight:160}}>
                      <textarea value={bookText} onChange={e=>setBookText(e.target.value)} placeholder="Write freely... no prompts, no rules." style={{width:"100%",height:"100%",minHeight:160,background:"transparent",border:"none",padding:"14px 16px",fontFamily:SERIF,fontSize:"0.88rem",color:"#E8DCC8",lineHeight:1.8,boxSizing:"border-box",resize:"none"}}/>
                    </div>
                    {bookText.trim()&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",marginTop:8,background:"rgba(201,169,110,0.08)",borderRadius:8,border:"1px solid rgba(201,169,110,0.15)"}}>
                      <span style={{fontSize:"0.7rem",color:"rgba(230,210,165,0.5)",fontFamily:SANS}}>{bookSaveMsg||`${wc(bookText)} words`}</span>
                ...
                {journalSection==="bible-notes"&&bookPage===2&&<>
                  <div style={{flex:1,display:"flex",flexDirection:"column",animation:"pageContentReveal .5s .1s ease both"}}>
                    ...
                    <h2 style={{fontFamily:DISPLAY,fontSize:"clamp(1.05rem,4vw,1.2rem)",fontWeight:700,color:"#F0E0C0",margin:"0 0 10px",textAlign:"center"}}>Bible Notes</h2>
                    <div style={{width:40,height:1,background:"linear-gradient(90deg,transparent,rgba(201,169,110,0.3),transparent)",margin:"0 auto 14px"}}/>
                    <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",paddingRight:2}}>
                      {bibleNotes.length===0&&(
                        <div style={{textAlign:"center",marginTop:40}}>
                          <p style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"0.9rem",color:"rgba(230,210,165,0.45)",margin:0}}>No notes yet</p>
                          <p style={{fontFamily:SANS,fontSize:"0.72rem",color:"rgba(230,210,165,0.35)",marginTop:8,lineHeight:1.5}}>While reading Scripture, select a verse and tap "Note" to save a reflection here.</p>
                        </div>
                      )}
                      {bibleNotes.map(n=>(
                        <div key={n.id} style={{background:"rgba(201,169,110,0.05)",border:"1px solid rgba(201,169,110,0.13)",borderRadius:12,padding:"14px 14px",marginBottom:12}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",gap:8,marginBottom:6}}>
                            <span style={{fontFamily:DISPLAY,fontSize:"0.84rem",fontWeight:700,color:"#E8DCC8"}}>{n.ref||"Note"}</span>
                            <span style={{fontFamily:SANS,fontSize:"0.6rem",color:"rgba(230,210,165,0.35)",whiteSpace:"nowrap"}}>{n.date}</span>
                          </div>
                          {n.text&&(
                          ...
                          <p style={{fontFamily:SANS,fontSize:"0.85rem",color:"#E8DCC8",lineHeight:1.65,margin:"0 0 10px",whiteSpace:"pre-wrap"}}>{n.note}</p>
                          <div style={{display:"flex",gap:8}}>
                            {openScriptureToRef&&n.bookIdx!=null&&(
                              <button onClick={()=>{setBookOpen(false);openScriptureToRef(n.bookIdx,n.chapIdx);}} style={{background:"rgba(201,169,110,0.10)",border:"1px solid rgba(201,169,110,0.18)",borderRadius:8,padding:"6px 14px",cursor:"pointer",color:"rgba(230,210,165,0.7)",fontFamily:SANS,fontSize:"0.7rem"}}>Read</button>
                            )}
                            {deleteBibleNote&&(
                ...
                {journalSection==="rooms"&&bookPage>=2&&<>
                  {(()=>{
                    ...
                    return<div style={{flex:1,display:"flex",flexDirection:"column",animation:"pageContentReveal .5s .1s ease both"}}>
                      ...
                      {roomIdx===0&&<>
                        <h2 style={{fontFamily:DISPLAY,fontSize:"clamp(1.05rem,4vw,1.2rem)",fontWeight:700,color:"#F0E0C0",margin:"0 0 10px",textAlign:"center"}}>Past Reflections</h2>
                        <div style={{width:40,height:1,background:"linear-gradient(90deg,transparent,rgba(201,169,110,0.3),transparent)",margin:"0 auto 12px"}}/>
                        {renderSectionHistory(
                      ...
                      {roomIdx>=1&&roomIdx<=REFLECTION_ROOMS.length&&(()=>{
                        ...
                        return<>
                          <div style={{textAlign:"center",marginBottom:14}}>
                            ...
                            <h2 style={{fontFamily:DISPLAY,fontSize:"clamp(1.15rem,4.5vw,1.35rem)",fontWeight:700,color:"#F0E0C0",margin:"0 0 4px"}}>{room.label}</h2>
                            <div style={{fontFamily:SANS,fontSize:"0.6rem",color:"rgba(230,210,165,0.5)",letterSpacing:"0.1em",textTransform:"uppercase",marginTop:3}}>{done?"Complete":`Day ${prog+1} of ${room.days.length}`}</div>
                          </div>
                          <div style={{width:40,height:1,background:"linear-gradient(90deg,transparent,rgba(201,169,110,0.3),transparent)",margin:"0 auto 16px"}}/>
                          <p style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"clamp(0.85rem,3vw,0.95rem)",color:"#E8DCC8",lineHeight:1.7,textAlign:"center",margin:"0 6px 8px"}}>"{room.question}"</p>
                          {!done&&<div style={{background:"rgba(201,169,110,0.06)",borderRadius:8,padding:"14px 16px",margin:"12px 0",border:"1px solid rgba(201,169,110,0.1)"}}>
                            <div style={{fontFamily:SANS,fontSize:"0.56rem",color:"rgba(230,210,165,0.45)",letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:6}}>Today's Prompt</div>
                            <p style={{fontFamily:SERIF,fontSize:"clamp(0.8rem,2.8vw,0.88rem)",color:"#E8DCC8",lineHeight:1.6,margin:0}}>{dayData.q}</p>
                            <p style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"0.72rem",color:"rgba(230,210,165,0.38)",margin:"6px 0 0"}}>{dayData.hint}</p>
                          </div>}
                          {done&&<div style={{background:"rgba(201,169,110,0.05)",borderRadius:8,padding:"12px 16px",margin:"12px 0",border:"1px dashed rgba(201,169,110,0.12)",textAlign:"center"}}>
                            <p style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"0.82rem",color:"rgba(230,210,165,0.45)",margin:0}}>You've completed all {room.days.length} days in this room.</p>
                          </div>}
                          <div style={{flex:1,minHeight:16}}/>
                          <button className="book-room" onClick={()=>{setBookOpen(false);enterRoom(room,"cabin");}} style={{alignSelf:"center",background:"linear-gradient(135deg,rgba(93,74,46,0.1),rgba(93,74,46,0.04))",border:"1px solid rgba(93,74,46,0.22)",color:"#E8DCC8",padding:"11px 32px",borderRadius:8,fontFamily:SERIF,fontStyle:"italic",fontSize:"0.84rem",cursor:"pointer",transition:"all .2s",letterSpacing:"0.02em"}}>{done?"Revisit this room":"Begin reflecting"}</button>
                        </>;
                      })()}
                      ...
                      {roomIdx===REFLECTION_ROOMS.length+1&&(()=>{
                        ...
                        <div style={{textAlign:"center",marginBottom:14}}>
                          ...
                          <h2 style={{fontFamily:DISPLAY,fontSize:"clamp(1.1rem,4.5vw,1.3rem)",fontWeight:700,color:"#F0E0C0",margin:"0 0 4px",textAlign:"center"}}>Questions Jesus Asked</h2>
                        </div>
                        <div style={{width:40,height:1,background:"linear-gradient(90deg,transparent,rgba(201,169,110,0.3),transparent)",margin:"0 auto 16px"}}/>
                        <p style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"clamp(0.88rem,3vw,1rem)",color:"#E8DCC8",lineHeight:1.7,textAlign:"center",margin:"0 4px"}}>"{jq.q}"</p>
                        <p style={{fontFamily:SANS,fontSize:"0.66rem",color:"rgba(230,210,165,0.4)",margin:"8px 0 0",textAlign:"center"}}>{jq.ref}</p>
                        <div style={{background:"rgba(201,169,110,0.06)",borderRadius:8,padding:"12px 16px",margin:"18px 0",border:"1px solid rgba(201,169,110,0.1)"}}>
                          <p style={{fontFamily:SERIF,fontSize:"0.82rem",color:"#E8DCC8",lineHeight:1.55,margin:0,textAlign:"center"}}>{jq.app}</p>
                        </div>
                        <div style={{flex:1}}/>
                        <button className="book-room" onClick={()=>{setBookOpen(false);setScreen("jesus");}} style={{alignSelf:"center",background:"linear-gradient(135deg,rgba(93,74,46,0.1),rgba(93,74,46,0.04))",border:"1px solid rgba(93,74,46,0.22)",color:"#E8DCC8",padding:"11px 32px",borderRadius:8,fontFamily:SERIF,fontStyle:"italic",fontSize:"0.84rem",cursor:"pointer",transition:"all .2s"}}>Open Scripture questions</button>
                      </>;})()}
                      ...
                      {roomIdx===REFLECTION_ROOMS.length+2&&(()=>{
                        ...
                        return<div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center"}}>
                          ...
                          <h2 style={{fontFamily:DISPLAY,fontSize:"clamp(1.1rem,4.5vw,1.3rem)",fontWeight:700,color:unlocked?"#F0E0C0":"rgba(240,224,192,0.35)",margin:"0 0 4px"}}>The Locked Room</h2>
                          <div style={{width:40,height:1,background:`linear-gradient(90deg,transparent,rgba(201,169,110,${unlocked?0.3:0.12}),transparent)`,margin:"12px auto 18px"}}/>
                          {unlocked?<>
                            <p style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"0.92rem",color:"#E8DCC8",lineHeight:1.7,margin:"0 8px 24px"}}>The deepest questions await.</p>
                            <button className="book-room" onClick={()=>{setBookOpen(false);enterRoom(LOCKED_ROOM,"cabin");}} style={{background:"linear-gradient(135deg,rgba(93,74,46,0.1),rgba(93,74,46,0.04))",border:"1px solid rgba(93,74,46,0.22)",color:"#E8DCC8",padding:"11px 32px",borderRadius:8,fontFamily:SERIF,fontStyle:"italic",fontSize:"0.84rem",cursor:"pointer"}}>Enter the locked room</button>
                          </>:<>
                            <p style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"0.88rem",color:"rgba(230,210,165,0.4)",lineHeight:1.7,margin:"0 8px"}}>Some rooms only open with consistency.</p>
                            <div style={{width:"60%",maxWidth:180,height:4,background:"rgba(201,169,110,0.1)",borderRadius:99,margin:"22px auto 8px",overflow:"hidden"}}><div style={{width:`${(streak/7)*100}%`,height:"100%",background:"linear-gradient(90deg,#C9A96E,#C9A96E)",borderRadius:99}}/></div>
                            <p style={{fontFamily:SANS,fontSize:"0.66rem",color:"rgba(230,210,165,0.35)"}}>{7-streak} more day{7-streak===1?"":"s"} to unlock</p>
                          </>}
                        </div>;
                      })()}
                      ...
                      {roomIdx===REFLECTION_ROOMS.length+3&&(()=>{
                        ...
                        return<>
                          <div style={{textAlign:"center",marginBottom:14}}>
                            <h2 style={{fontFamily:DISPLAY,fontSize:"clamp(1.05rem,4vw,1.2rem)",fontWeight:700,color:"#F0E0C0",margin:"0 0 4px"}}>Question to Carry Today</h2>
                          </div>
                          <div style={{width:40,height:1,background:"linear-gradient(90deg,transparent,rgba(201,169,110,0.3),transparent)",margin:"0 auto 16px"}}/>
                          <div style={{background:"rgba(201,169,110,0.06)",borderRadius:8,padding:"18px 16px",border:"1px solid rgba(201,169,110,0.1)",marginBottom:14}}>
                            <p style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"clamp(0.88rem,3vw,0.98rem)",color:"#E8DCC8",lineHeight:1.7,margin:0,textAlign:"center"}}>{dailyQ}</p>
                          </div>
                          <button className="book-room" onClick={()=>{setBookOpen(false);setCardQ(dailyQ);setIsCustomCard(false);setScreen("cards");}} style={{alignSelf:"center",background:"transparent",border:"1px solid rgba(101,83,55,0.2)",color:"#E8DCC8",padding:"9px 22px",borderRadius:8,fontFamily:SERIF,fontStyle:"italic",fontSize:"0.78rem",cursor:"pointer"}}>Make a card</button>
                        </>;
                      })()}
                ...
                {journalSection==="dreams"&&bookPage>=2&&(()=>{
                  ...
                  if(bookPage===2) return<div style={{flex:1,display:"flex",flexDirection:"column",animation:"pageContentReveal .5s .1s ease both"}}>
                    ...
                    <h2 style={{fontFamily:DISPLAY,fontSize:"clamp(1.05rem,4vw,1.2rem)",fontWeight:700,color:"#F0E0C0",margin:"0 0 10px",textAlign:"center"}}>Dream History</h2>
                    <div style={{width:40,height:1,background:"linear-gradient(90deg,transparent,rgba(201,169,110,0.3),transparent)",margin:"0 auto 12px"}}/>
                    {renderSectionHistory(
                  ...
                  return<div style={{flex:1,display:"flex",flexDirection:"column",animation:"pageContentReveal .5s .1s ease both"}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
                      <button onClick={()=>{setJournalSection(null);setBookPage(1);setFlipDir("bwd");}} style={{background:"transparent",border:"none",cursor:"pointer",fontFamily:SERIF,fontSize:"0.74rem",color:"rgba(230,210,165,0.5)",padding:0}}>&#8249; Contents</button>
                      <div style={{flex:1}}/>
                      <span style={{fontFamily:SANS,fontSize:"0.58rem",color:"rgba(230,210,165,0.35)",letterSpacing:"0.1em",textTransform:"uppercase"}}>Dream {pgIdx+1} of {dreamPages.length}</span>
                    </div>
                    <h2 style={{fontFamily:DISPLAY,fontSize:"clamp(1.15rem,4.5vw,1.35rem)",fontWeight:700,color:"#F0E0C0",margin:"0 0 4px",textAlign:"center"}}>{pg.title}</h2>
                    <div style={{width:40,height:1,background:"linear-gradient(90deg,transparent,rgba(201,169,110,0.3),transparent)",margin:"8px auto 16px"}}/>
                    <div style={{background:"rgba(201,169,110,0.06)",borderRadius:8,padding:"16px 18px",margin:"0 0 10px",border:"1px solid rgba(201,169,110,0.1)"}}>
                      <p style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"clamp(0.88rem,3vw,0.98rem)",color:"#E8DCC8",lineHeight:1.7,margin:0,textAlign:"center"}}>{pg.prompt}</p>
                    </div>
                    <p style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"0.74rem",color:"rgba(230,210,165,0.4)",textAlign:"center",margin:"0 0 14px",lineHeight:1.6}}>{pg.hint}</p>
                    <div style={{background:"rgba(201,169,110,0.04)",border:"1px solid rgba(201,169,110,0.1)",borderRadius:10,overflow:"hidden",flex:1,minHeight:120}}>
                      <textarea value={bookText} onChange={e=>setBookText(e.target.value)} placeholder="Write your thoughts here..." style={{width:"100%",height:"100%",minHeight:120,background:"transparent",border:"none",padding:"14px 16px",fontFamily:SERIF,fontSize:"0.88rem",color:"#E8DCC8",lineHeight:1.8,boxSizing:"border-box",resize:"none"}}/>
                    </div>
                    {bookText.trim()&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",marginTop:8,background:"rgba(201,169,110,0.08)",borderRadius:8,border:"1px solid rgba(201,169,110,0.15)"}}>
                      <span style={{fontSize:"0.7rem",color:"rgba(230,210,165,0.5)",fontFamily:SANS}}>{bookSaveMsg||`${wc(bookText)} words`}</span>
                ...
                {journalSection==="prayers"&&bookPage>=2&&<>
                  <div style={{flex:1,display:"flex",flexDirection:"column",animation:"pageContentReveal .5s .1s ease both"}}>
                    ...
                    {bookPage===2&&<>
                      <h2 style={{fontFamily:DISPLAY,fontSize:"clamp(1.05rem,4vw,1.2rem)",fontWeight:700,color:"#F0E0C0",margin:"0 0 10px",textAlign:"center"}}>Your Prayers</h2>
                      <div style={{width:40,height:1,background:"linear-gradient(90deg,transparent,rgba(201,169,110,0.3),transparent)",margin:"0 auto 12px"}}/>
                      {renderSectionHistory(
                    ...
                    {bookPage===3&&<>
                      <h2 style={{fontFamily:DISPLAY,fontSize:"clamp(1.1rem,4vw,1.3rem)",fontWeight:700,color:"#F0E0C0",margin:"0 0 4px",textAlign:"center"}}>Write a Prayer</h2>
                      <p style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"0.78rem",color:"rgba(230,210,165,0.45)",textAlign:"center",margin:"4px 0 14px"}}>Each prayer waters a plant in your garden</p>
                      <div style={{width:40,height:1,background:"linear-gradient(90deg,transparent,rgba(201,169,110,0.3),transparent)",margin:"0 auto 14px"}}/>
                      <div style={{background:"rgba(201,169,110,0.04)",border:"1px solid rgba(201,169,110,0.1)",borderRadius:10,overflow:"hidden",flex:1,minHeight:160}}>
                        <textarea value={bookText} onChange={e=>setBookText(e.target.value)} placeholder="Pour out your heart... He is listening." style={{width:"100%",height:"100%",minHeight:160,background:"transparent",border:"none",padding:"14px 16px",fontFamily:SERIF,fontSize:"0.88rem",color:"#E8DCC8",lineHeight:1.8,boxSizing:"border-box",resize:"none"}}/>
                      </div>
                      {bookText.trim()&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",marginTop:8,background:"rgba(201,169,110,0.08)",borderRadius:8,border:"1px solid rgba(201,169,110,0.15)"}}>
                        <span style={{fontSize:"0.7rem",color:"rgba(230,210,165,0.5)",fontFamily:SANS}}>{bookSaveMsg||`${wc(bookText)} words`}</span>
              ...
              {deskBook!=="journal"&&BOOK_CONTENT[deskBook]&&<>
                ...
                {bookPage===0&&(deskBook==="gratitude"?(
                  ...
                  <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center",animation:"pageContentReveal .5s .15s ease both"}}>
                    ...
                    <h2 style={{fontFamily:DISPLAY,fontSize:"clamp(1.3rem,5vw,1.6rem)",fontWeight:700,color:"#F0E0C0",margin:"0 0 6px",letterSpacing:"0.02em"}}>{BOOK_CONTENT[deskBook].cover.title}</h2>
                    <div style={{width:50,height:1,background:"linear-gradient(90deg,transparent,#C9A96E,transparent)",margin:"4px auto 18px"}}/>
                    <p style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"clamp(0.88rem,3vw,1rem)",color:"#D8C4A0",lineHeight:1.75,maxWidth:280,margin:"0 auto"}}>{BOOK_CONTENT[deskBook].cover.subtitle}</p>
                ...
                {bookPage>=1&&bookPage<=BOOK_CONTENT[deskBook].pages.length&&(()=>{
                  ...
                  return<>
                    <div style={{flex:1,display:"flex",flexDirection:"column",animation:"pageContentReveal .5s .1s ease both"}}>
                      <div style={{textAlign:"center",marginBottom:14}}>
                        <div style={{fontFamily:SANS,fontSize:"0.56rem",color:"rgba(230,210,165,0.45)",letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:8}}>Page {bookPage}</div>
                        <h2 style={{fontFamily:DISPLAY,fontSize:"clamp(1.15rem,4.5vw,1.35rem)",fontWeight:700,color:"#F0E0C0",margin:"0 0 4px"}}>{pg.title}</h2>
                      </div>
                      <div style={{width:40,height:1,background:"linear-gradient(90deg,transparent,rgba(201,169,110,0.3),transparent)",margin:"0 auto 16px"}}/>
                      <div style={{background:"rgba(201,169,110,0.06)",borderRadius:8,padding:"16px 18px",margin:"0 0 14px",border:"1px solid rgba(201,169,110,0.1)"}}>
                        <p style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"clamp(0.88rem,3vw,0.98rem)",color:"#E8DCC8",lineHeight:1.7,margin:0,textAlign:"center"}}>{pg.prompt}</p>
                      </div>
                      <p style={{fontFamily:SERIF,fontStyle:"italic",fontSize:"0.74rem",color:"rgba(230,210,165,0.4)",textAlign:"center",margin:"0 0 16px",lineHeight:1.6}}>{pg.hint}</p>
                      {/* Writing area */}
          ...
          {/* Nav arrows */}
          {bookPage>0&&<button className="book-nav" onClick={()=>flipPage("bwd")} style={{position:"absolute",left:-20,top:"50%",transform:"translateY(-50%)",width:38,height:38,borderRadius:"50%",background:"rgba(30,23,16,0.78)",backdropFilter:"blur(4px)",WebkitBackdropFilter:"blur(4px)",border:"1px solid rgba(201,169,110,0.3)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1rem",color:"#E8DCC8",zIndex:10,boxShadow:"0 2px 12px rgba(0,0,0,0.25)"}}>‹</button>}
